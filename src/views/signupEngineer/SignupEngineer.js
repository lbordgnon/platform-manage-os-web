...
export const SignupEngineer = ({ title, subtitle, subtext }) => {
  ...
  const [showErrorFederalId, setShowErrorFederalId] = useState(false);
  ...
  const signup = async () => {
    await EngineerService.signupEngineer(name, email, phone,federalId,password)
  ...
  const validateFederalId= () => {
    if (federalId === '' || federalId.length !== 11) {
      setShowErrorFederalId(true);
      setDisableButton(true);
    } else {
      setShowErrorFederalId(false);
    }
  };
  ...
  const HandleChangeFederalId = (value) => {
    let federalId = value.replace(/[^\d]+/g, '');
    if (federalId.length > 11) {
      federalId = federalId.slice(0, -1);
    }
    ...
    <div>
      ...
      <Box>
        <Stack mb={3}>
          ...
          <CustomTextField
            error={showErrorFederalId}
            helperText={showErrorFederalId ? 'Campo de preenchimento obrigatório' : ''}
            id="name"
            variant="outlined"
            fullWidth
            ...
            maxLength="11"
            value={federalId}
            onBlur={() => validateFederalId()}
            onChange={(e) => {
              HandleChangeFederalId(e.target.value);
            }}
          />
